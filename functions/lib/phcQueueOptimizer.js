...
exports.PHC_QUEUE_CONFIG = void 0;
exports.calculateErlangC = calculateErlangC;
exports.getExpectedConsultationDuration = getExpectedConsultationDuration;
exports.calculateDynamicPriorityScore = calculateDynamicPriorityScore;
exports.simulateMultiServerWaitTimes = simulateMultiServerWaitTimes;
exports.evaluateEmergencyEscalationCriteria = evaluateEmergencyEscalationCriteria;
exports.optimizePhcPatientQueue = optimizePhcPatientQueue;
exports.persistOptimizedQueueToFirestore = persistOptimizedQueueToFirestore;
// ============================================================================
// CONFIGURATION & CONSTANTS FOR PRIMARY HEALTH CENTRES
// ============================================================================
exports.PHC_QUEUE_CONFIG = {
 ...
 */
function calculateErlangC(c, offeredLoad) {
    if (c <= 0 || offeredLoad <= 0)
        return 0;
    const rho = offeredLoad / c;
    if (rho >= 1)
        return 1; // Unstable or saturated queue
    let sum = 0;
    for (let n = 0; n < c; n++) {
 ...
 */
function getExpectedConsultationDuration(esiTier, department, isEmergencyEscalated) {
    if (isEmergencyEscalated || esiTier === 1) {
        return exports.PHC_QUEUE_CONFIG.defaultEmergencyConsultMinutes;
    }
    if (esiTier === 2) {
        return 20;
    }
    if (esiTier === 3) {
        return exports.PHC_QUEUE_CONFIG.defaultUrgentConsultMinutes;
    }
    if (department.toLowerCase().includes('maternal') || department.toLowerCase().includes('anc')) {
        return exports.PHC_QUEUE_CONFIG.defaultAncMaternalConsultMinutes;
    }
    return exports.PHC_QUEUE_CONFIG.defaultRoutineConsultMinutes;
}
/**
 * Calculates dynamic priority score with aging to prevent starvation
 */
function calculateDynamicPriorityScore(params) {
    ...
    if (isEmergencyEscalated || esiTier === 1) {
        return exports.PHC_QUEUE_CONFIG.emergencyBaseScore + urgencyScore * 10 + waitingDurationMinutes;
    }
    ...
    const agingScore = waitingDurationMinutes * exports.PHC_QUEUE_CONFIG.agingRatePerMinute;
    // Slight appointment bonus over walk-ins
    const apptScore = !isWalkIn ? exports.PHC_QUEUE_CONFIG.appointmentBonusScore : 0;
 ...
 */
function simulateMultiServerWaitTimes(waitingItemsSorted, rooms) {
    ...
    for (const patient of waitingItemsSorted) {
        ...
        for (let i = 1; i < serverNextAvailableMinutes.length; i++) {
            if (serverNextAvailableMinutes[i].availableAt <
 ...
 */
function evaluateEmergencyEscalationCriteria(vitals, chiefComplaint, age, notes) {
    const redFlags = [];
    const actions = [];
    ...
    if (fullText.includes('chest pain') ||
    ...
    if (fullText.includes('postpartum') ||
    ...
    if (fullText.includes('anaphylaxis') ||
    ...
    else if (vitals.temperature && vitals.temperature >= 102.5 ||
 ...
 */
function optimizePhcPatientQueue(params) {
    const { facilityId, facilityName, rawQueueItems, rooms = [], escalatedPatientId, emergencyReason, currentTime = new Date(), } = params;
    ...
    for (const inConsult of inConsultationItems) {
        ...
        if (room && inConsult.consultationStartTime) {
            ...
            const elapsedMinutes = Math.max(0, Math.round((nowMs - new Date(inConsult.consultationStartTime).getTime()) / 60000));
    ...
    const waitingItems = rawQueueItems.filter((q) => q.status === 'waiting' || q.status === 'escalated');
    let emergencyAuditLog;
    ...
    for (const item of waitingItems) {
        ...
        item.expectedConsultationDuration = getExpectedConsultationDuration(item.esiTier || 4, item.department || 'General Outpatient', Boolean(item.isEmergencyEscalated));
    ...
    const { estimatedWaitTimes, roomAssignments } = simulateMultiServerWaitTimes(waitingItems, activeRooms);
    ...
    const avgDurationMins = waitingItems.length > 0
        ? waitingItems.reduce((acc, q) => acc + q.expectedConsultationDuration, 0) /
            waitingItems.length
        : exports.PHC_QUEUE_CONFIG.defaultRoutineConsultMinutes;
    ...
    const theoreticalWaitTimeMinutes = trafficIntensity < 1
        ? Math.round((erlangC * avgDurationMins) / (c * (1 - trafficIntensity)))
        : Math.round(totalWaiting * (avgDurationMins / c));
    const averageWaitTimeMinutes = waitingItems.length > 0
        ? Math.round(waitingItems.reduce((acc, q) => acc + q.estimatedWaitMinutes, 0) / waitingItems.length)
        : 0;
    // Re-combine full list (waiting + in_consultation + completed)
    const nonWaitingItems = rawQueueItems.filter((q) => q.status !== 'waiting' && q.status !== 'escalated');
 ...
 */
async function persistOptimizedQueueToFirestore(db, optimizationResult) {
    try {
        const batch = db.batch();
        const itemsToUpdate = optimizationResult.optimizedQueue.filter((q) => q.status === 'waiting' || q.status === 'escalated');
...
//# sourceMappingURL=phcQueueOptimizer.js.map