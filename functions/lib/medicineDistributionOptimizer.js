...
exports.LOGISTICS_COST_PARAMS = exports.RURAL_BARRIER_SPECS = void 0;
exports.calculateHaversineDistanceKm = calculateHaversineDistanceKm;
exports.estimateRoadDistanceKm = estimateRoadDistanceKm;
exports.detectGeographicBarrier = detectGeographicBarrier;
exports.calculateTransportationCostCell = calculateTransportationCostCell;
exports.solveVogelsApproximationMethod = solveVogelsApproximationMethod;
exports.buildOptimizedTransitRoutes = buildOptimizedTransitRoutes;
exports.optimizeMedicineDistribution = optimizeMedicineDistribution;
exports.persistDistributionPlanToFirestore = persistDistributionPlanToFirestore;
// ============================================================================
// 1. RURAL GEOGRAPHIC BARRIERS CONFIGURATION
// ============================================================================
exports.RURAL_BARRIER_SPECS = {
    mountain_ghat: {
 ...
 */
function estimateRoadDistanceKm(straightDistanceKm, barrierType) {
 ...
 */
function calculateTransportationCostCell(supply, demand) {
    const straightDistanceKm = calculateHaversineDistanceKm(supply.coordinates.latitude, supply.coordinates.longitude, demand.coordinates.latitude, demand.coordinates.longitude);
    const barrierType = detectGeographicBarrier({ facilityId: supply.facilityId, name: supply.facilityName, coordinates: supply.coordinates }, { facilityId: demand.facilityId, name: demand.facilityName, tier: demand.tier, barrierFromSupply: demand.barrierFromSupply });
    const spec = exports.RURAL_BARRIER_SPECS[barrierType];
    ...
    let kmRate = exports.LOGISTICS_COST_PARAMS.baseCostPerKmInr * spec.costMultiplier;
    if (demand.coldChainRequired) {
        kmRate += exports.LOGISTICS_COST_PARAMS.coldChainSurchargePerKmInr;
    }
    const distanceCost = roadDistanceKm * kmRate;
    const timeCost = transitMinutes * exports.LOGISTICS_COST_PARAMS.driverMinuteCostInr;
    const totalTripCost = distanceCost + timeCost + exports.LOGISTICS_COST_PARAMS.fixedDepotHandlingFeeInr;
 ...
 */
function solveVogelsApproximationMethod(supplies, demands, costMatrix) {
 ...
 */
function buildOptimizedTransitRoutes(supplies, demands, allocations) {
    const routes = [];
    // Group allocations by supply depot
    const allocationsBySupply = {};
    ...
    for (const [supplyId, depotAllocs] of Object.entries(allocationsBySupply)) {
        ...
        if (!supply || depotAllocs.length === 0)
            continue;
        ...
        const hasColdChain = depotAllocs.some((a) => demandMap.get(a.demandId)?.coldChainRequired);
        const hasRoughTerrain = depotAllocs.some((a) => a.barrierType === 'mountain_ghat' || a.barrierType === 'unpaved_rural');
        let vehicleType = 'standard_distribution_truck';
        let capacity = 1500;
        if (hasColdChain) {
        ...
        while (unvisited.length > 0) {
            ...
            for (let i = 0; i < unvisited.length; i++) {
                ...
                if (!targetDemand)
                    continue;
                const dist = calculateHaversineDistanceKm(currentLat, currentLon, targetDemand.coordinates.latitude, targetDemand.coordinates.longitude);
                const barrier = detectGeographicBarrier({ facilityId: supply.facilityId, coordinates: { latitude: currentLat, longitude: currentLon } }, { facilityId: targetDemand.facilityId, name: targetDemand.facilityName, tier: targetDemand.tier, barrierFromSupply: targetDemand.barrierFromSupply });
                const spec = exports.RURAL_BARRIER_SPECS[barrier];
        ...
        let cumDistance = 0;
        let cumMinutes = 0;
        const barriersTraversedSet = new Set();
        for (let order = 0; order < orderedAllocs.length; order++) {
            ...
            if (!targetDemand)
                continue;
            const segStraightKm = calculateHaversineDistanceKm(prevLat, prevLon, targetDemand.coordinates.latitude, targetDemand.coordinates.longitude);
            const barrier = detectGeographicBarrier({ facilityId: supply.facilityId, coordinates: { latitude: prevLat, longitude: prevLon } }, { facilityId: targetDemand.facilityId, name: targetDemand.facilityName, tier: targetDemand.tier, barrierFromSupply: targetDemand.barrierFromSupply });
 ...
 */
function optimizeMedicineDistribution(input) {
    ...
    for (const s of allStops) {
        if (s.barrierEncountered === 'mountain_ghat') {
            mountainCount++;
            terrainDelayTotal += exports.RURAL_BARRIER_SPECS.mountain_ghat.fixedDelayMinutes;
        }
        else if (s.barrierEncountered === 'unpaved_rural') {
            unpavedCount++;
            terrainDelayTotal += exports.RURAL_BARRIER_SPECS.unpaved_rural.fixedDelayMinutes;
        }
        else if (s.barrierEncountered === 'river_crossing') {
            riverCount++;
            terrainDelayTotal += exports.RURAL_BARRIER_SPECS.river_crossing.fixedDelayMinutes;
        }
        else {
            highwayCount++;
        }
    }
    const satisfactionRate = totalDemandUnits > 0
    ...
    return {
        ...
        averageTransitTimePerStopMinutes: allStops.length > 0 ? Math.round(totalTransit / allStops.length) : 0,
 ...
 */
async function persistDistributionPlanToFirestore(db, plan) {
    try {
...
//# sourceMappingURL=medicineDistributionOptimizer.js.map